...
def get_successor_states(state, action, domain, raise_error_on_invalid_action=False,
    ...
    return get_successor_state(state, action, domain, raise_error_on_invalid_action, inference_mode,
                               require_unique_assignment, get_all_transitions=True, return_probs=return_probs)
...
def _apply_effects(state, lifted_effects, assignments, get_all_transitions=False,
    ...
    for prob_efs_combination in probabilistic_effects_combinations:
        ...
        new_prob_literals = _compute_new_state_from_lifted_effects(new_determinized_lifted_effects, assignments,
                                                                   new_prob_literals)
...
class PDDLEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.state_dict[self.state_idx] = self.get_state()

        return self.get_obs()#self.state_idx
    ...
    def get_state_idx(self, state):
        if state in self.state_dict.values():
            for idx, s in self.state_dict.items():
                if s == state:
                    return idx
        else:
            self.state_idx += 1
            self.state_dict[self.state_idx] = state
            return self.state_idx

    def get_state_from_idx(self, idx):
        if idx in self.state_dict.keys():
            return self.state_dict[idx]
        else:
            raise Exception(f"No state found with idx: {idx}")

    def sample_action_space(self, obs):
        state = self.get_state_from_idx(self.state_idx)
        action = self.action_space.sample(state)
        return action
    ...
    def step(self, action):
        ...
        state = self.get_state_idx(state)
        
        return self.get_obs(), reward, done, debug_info
