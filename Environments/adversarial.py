...
Has additional functions, step_generator, and reset. How to use:
1. Call reset() to reset to an empty environment
2. Call step_generator() to place the goal, agent, and obstacles. Repeat until
   a done is received.
3. Normal RL loop. Use learning agent to generate actions and use them to call
   step() until a done is received.
4. If required, call reset() to reset the environment the way the
...
class AdversarialEnv(multigrid.MultiGridEnv, Base_Env):
    ...
    def __init__(self, n_clutter=50, size=15, agent_view_size=5, max_steps=250,
        ...
        Args:
          ...
          goal_noise: The probability with which the goal will move to a different
            location than the one chosen by the adversary.f
    ...
    def reset_random(self):
        ...
        self.adversary_step_count = self.n_clutter/2
...
register_env(AdversarialEnv)
register_env(ReparameterizedAdversarialEnv)
register_env(MiniAdversarialEnv)
register_env(MiniReparameterizedAdversarialEnv)
register_env(NoisyAdversarialEnv)
register_env(MediumAdversarialEnv)
register_env(GoalLastAdversarialEnv)
register_env(AdversarialEnv)
register_env(MiniGoalLastAdversarialEnv)
